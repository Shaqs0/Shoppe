		...
		<div className="flex min-h-[80vh] items-center justify-center">
			<div className="w-full max-w-md">
				...
				<div className="mt-16 flex justify-center space-x-4 bg-[#EFEFEF]">
					<button
						className={`w-1/2 rounded-lg px-5 py-2 ${isSignIn ? 'bg-[white]' : 'bg-[gray-100]'}`}
						onClick={() => setIsSignIn(true)}
					>
            Sign in
					</button>
					<button
						className={`w-1/2 rounded-lg px-5 py-2 ${!isSignIn ? 'bg-[white]' : 'bg-[#EFEFEF]'}`}
					...
					{!isSignIn && (
						<>
							...
							<div className="relative">
								...
								/>
								{errors.patronymic && (
									<p className="mt-1 text-sm text-[red-500]">{errors.patronymic.message}</p>
								)}
							</div>
						</>
					)}
